...
class DatabaseInitializer(private val db: AppDatabase) {
    ...
    suspend fun initialize() {
        ...
        val adminId = allUsers.find { it.username == "admin" }?.id

        // Seed Default Fee Rule
        val feeRuleDao = db.feeRuleDao()
        if (feeRuleDao.getCount() == 0) {
            feeRuleDao.insert(
                FeeRuleEntity(
        ...
        if (!hasNewOrders) {
            ...
            packageDao.insert(PackageEntity(deliveryRequestId = req1Id, name = "Cơm gà", weightKg = 0.5, quantity = 1, notes = "Nhiều cơm"))
            packageDao.insert(PackageEntity(deliveryRequestId = req1Id, name = "Coca Cola", weightKg = 0.3, quantity = 2))
            historyDao.insert(
                StatusHistoryEntity(
                    deliveryRequestId = req1Id,
                    fromStatus = null,
        ...
        if (!hasActiveOrders) {
            ...
            packageDao.insert(PackageEntity(deliveryRequestId = req2Id, name = "Phở Bò", weightKg = 0.6, quantity = 2))
            historyDao.insert(
                StatusHistoryEntity(
                    deliveryRequestId = req2Id,
                    fromStatus = null,
                    toStatus = DeliveryStatus.CHO_TIEP_NHAN,
                    updatedBy = client2Id,
                    note = "Đơn hàng được tạo"
